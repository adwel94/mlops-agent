...
```
task 환경  →  행동 궤적(.h5)  →  이미지 입힌 데이터셋(.h5)  →  영상 / 뷰어로 확인
```

task 환경에서 로봇 팔의 행동 궤적(.h5)을 만들고, 거기에 카메라 이미지를 입혀 (이미지 + 액션) 데이터셋을 자동으로 만들어주며, 결과를 영상·뷰어로 확인할 수 있다. 각 작업은 **슬래시 커맨드 / Python 함수 / CLI / Jupyter 노트북** 어느 방식으로든 동일하게 호출 가능.

## 무엇을 할 수 있나

- 행동 궤적(.h5) 얻기 — 공식 데모를 한 줄로 **받아오거나**(`fetch_sample_h5`), 모션 플래닝으로 **직접 생성**(`task_to_h5`)
- 그 궤적을 리플레이하면서 카메라 이미지를 입힌 데이터셋 자동 생성 (N 에피소드 배치, `h5_add_images`)
- 한 에피소드를 MP4로 추출하거나 여러 에피소드의 프레임 그리드 이미지 생성 (`h5_to_media`)
...
① 행동 궤적(.h5) 만들기 — 택1
   ...
   task_to_h5        모션 플래닝으로 직접 생성 (WSL)         ┘ → data/datasets/<task>/*.h5
        ↓
② h5_add_images      리플레이하며 카메라 이미지 입힘 → 데이터셋 → h5_to_media / view_task / replay_h5
```
...
:: ③ 0번 에피소드 MP4로 추출 (출력은 프로젝트 내부 data\ 에 저장됨)
python scripts\h5_to_media.py video --traj-path data\datasets\PickCube-v1\motionplanning.rgb.pd_joint_pos.physx_cpu.h5 --episode 0

:: ③ 또는 데스크탑 뷰어로 재생
python scripts\replay_h5.py --traj-path data\datasets\PickCube-v1\motionplanning.h5 --episode 0
```

Claude Code 안에서는 위 단계가 그대로 슬래시 커맨드: `/fetch_sample_h5`, `/task_to_h5`, `/h5_add_images`, `/h5_to_media`, `/view_task`, `/replay_h5`, `/check_maniskill_env`.
...
| 미디어 추출 | `/h5_to_media` | `from scripts.h5_to_media import video, preview` | `python scripts/h5_to_media.py video\|preview ...` |

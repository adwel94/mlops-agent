...
파인튜닝·평가하는 하네스. Windows(WSL 로 mplib 구동)와 macOS(Apple Silicon 네이티브,
pinocchio IK + MoltenVK 렌더)에서 돈다 — `task_to_h5`(mplib 모션 플래너)만 Windows+WSL 전용.
...
| [pinocchio](https://github.com/stack-of-tasks/pinocchio) | macOS 네이티브 IK 백엔드 — EE 실행(`ee_verify`·`gr00t_eval`)을 WSL 없이 돌린다 (`scripts/ik_exec.py` 가 가용성으로 선택) |
| Mesa lavapipe · MoltenVK (Vulkan) | GPU Vulkan 없는 곳에서 sapien 렌더를 받치는 드라이버 — WSL은 lavapipe(소프트웨어), macOS는 MoltenVK(Metal 변환) |
...
**홀드아웃 평가셋** (train/test 분리): 위 플로우는 *학습셋*을 만든다. 평가(`/gr00t_eval`)는
학습에 **안 쓴 씬**에서 재야 성공률이 정직하므로, 학습셋과 겹치지 않는 seed 구간에서 작은
세트를 따로 만들어 평가 입력으로 쓴다 (lerobot/hf 변환 불필요):

```
/task_to_h5 <task> 50 --start-seed 90000 --traj-name holdout  →  /h5_add_images  →  (/gr00t_eval 입력)
```

학습 씬에서 잰 값 = **eval v1**(낙관 편향), 홀드아웃 = **eval v2**(배포 근사) — MANIFEST `eval_method`
로 구분하며 둘은 사과-오렌지(직접 비교 금지).
...
| `/ee_verify` | `scripts/ee_verify.py` | EE-델타를 IK로 재현하는 게이트 (GR00T 변환 전 방법 확정; Windows는 WSL 구동, macOS는 네이티브) |
...
①의 데이터셋으로 실제 GPU에서 파인튜닝하고, 정책을 sim에 붙여 **홀드아웃 씬**에서 태스크 성공률을 잰다.
...
| `/gr00t_eval` | `scripts/gr00t_eval.py` | 정책을 sim에 붙여 닫힌 루프 롤아웃 → 태스크 성공률. **홀드아웃 h5**(학습 안 쓴 seed) 입력 (Windows는 WSL 구동, macOS는 네이티브) |
| `/gr00t_play` | `scripts/gr00t_play.py` | MANIFEST 좌표(`<task>/<model>`)의 모델을 뷰어 창에서 자연어 REPL 로 라이브 조종 (macOS 네이티브; 서버는 gr00t_serve) |

...
- **역할 분담** — AI: 코드·빌드·로컬 분석. 사용자: 시크릿·콘솔 로그·승인. 모니터링은 파드가 push(Discord/W&B)하고 AI는 *가끔* 조회(턴 기반 — 백그라운드 데몬 아님). AI 측 **자연어 리포트(해석·판단)** 는 **`#ai-리포팅`(AI_REPORTING) 채널로만** push(`ai_report.py "메시지"`; 숫자는 `wandb_status.py` 로 보고 prose 에 녹임) — 파드-push 채널(RUNPOD/PIPELINE/STDOUT)과 분리. **의미 있는 순간(시작·주목할 추세변화·완료·이상)에만**, 매 폴링 도배 금지.

...
- **카메라는 데이터에서 발견**(`modality.json` video 키), 하드코딩 금지 — `h5_to_lerobot`(`select_cameras`)·`new_embodiment_config`(`_discover_video_keys`, DATASET_DIR/meta/modality.json)·`_wsl_gr00t_eval` 이 모두 거기서 읽어 1캠/멀티캠(손목캠) 태스크 무관 처리. 체크포인트는 카메라 수 고정이라 v1(1캠)·v2(2캠)는 HF 태그로 분리(`notes/wrist-camera-impl.md`).

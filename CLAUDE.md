...
에피소드 = 한 번의 완결 시연 · 프레임 = 시뮬 한 타임스텝(env.step, 데이터 양의 단위) ·
스텝 = training step(가중치 1회 갱신, `max_steps`·사다리). "스텝"은 항상 학습스텝을 뜻하고,
시뮬/궤적 쪽은 "프레임"이라 부른다(1 스텝 = batch 프레임). 에포크 = 데이터 몇 바퀴
(= 스텝 × batch ÷ total_frames). GR00T 는 스트리밍이라 학습 로그의 `epoch` 필드는
step/max_steps 진행률일 뿐 — 데이터 커버리지는 위 식(프레임)으로 센다.

...
2. **과금은 승인, 과금 중단은 자율** — 과금을 *발생*시키는 행동(파드 생성·이미지 push·
   ...
   자율. 단, `plan_create` 계획서가 명시적 한도(`budget.max_usd`·`budget.max_train_runs`)와
   `approval_mode: pre_approved` 를 선언했으면, 그 계획을 `plan_run` 으로 실행하는 동안의 파드
   생성은 그 한도 내에서 **사전 승인된 것으로 본다**(한도 도달 시 멈추고 재승인). 이 예외는
   `plan_run` 실행 컨텍스트에 한정 — 그 밖의 개별 파드 생성은 건건 승인 그대로.

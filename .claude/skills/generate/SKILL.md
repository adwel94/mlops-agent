---
name: generate
description: 액션 궤적을 Windows CPU 시뮬에서 리플레이해 VLA 학습용 데이터셋(RGB 프레임 + 액션 HDF5)을 생성. 액션 공급원(fetch로 받은 데모 또는 solve로 생성한 궤적) 무엇이든 입력으로 받는 공유 소비 단계. args는 `[태스크-ID] [에피소드-수]` + 옵션 플래그 형식; 비어있으면 `PickCube-v1 100` 기본값. 기본 입력은 fetch가 받아둔 데모 — 못 찾으면 사용자에게 먼저 `/fetch <태스크>`(또는 `/solve <태스크>`)를 실행하라고 알릴 것.
---

# Generate 스킬 — VLA 데이터셋 생성 (리플레이)

이 프로젝트의 `scripts/generate.py` 래퍼. 액션 공급원(`fetch`로 받은 공식 데모 또는 `solve`로 생성한 궤적)이 만든 원본 궤적을 Windows CPU 시뮬/렌더 경로로 리플레이하면서 요청한 관측/컨트롤 모드로 새 HDF5에 기록한다. 두 공급원 모두 같은 형식(액션+states)을 뱉으므로 이 단계는 출처와 무관하게 동일.
...
- 기본 입력은 `~/.maniskill/demos/<task>/motionplanning/trajectory.h5` (fetch가 받아둔 것). 없으면 `FileNotFoundError` — 재시도 말고 사용자에게 먼저 `/fetch <태스크>`(받아오기) 또는 `/solve <태스크>`(직접 생성) 안내.
- `solve`로 만든 궤적(`data/datasets/<task>/motionplanning.h5`)을 입력으로 쓰려면 `--traj-path` 로 그 경로를 지정 (함수 호출 시 `traj_path=...`).

...
- **SOURCE 이미지 데이터셋 h5** (`h5_add_images` 출력) — `obs/extra/tcp_pose`, `episode_seed`, `label_metadata` 필요. LeRobot 디렉토리는 안 됨(seed/label이 없음).
...
- **순서**: `h5_to_lerobot`(포장) → `gr00t_train`(학습) → `gr00t_serve`(서빙) → **`gr00t_eval`**(평가). 입력은 LeRobot 변환에 쓴 **소스 h5와 같은 파일**.

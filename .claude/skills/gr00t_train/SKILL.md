...
학습 파드는 하네스가 못 추적하는 외부 상태 — 자동 알림이 없으니 직접 폴링한다.

- **폴링 방식**: `Bash` `run_in_background` 로 `sleep <N> && <확인>`(확인 = `wandb_status.py` /
  `read_logs.py` / `runpod_ls.py`) → task-notification 으로 결과 읽고 다음 수 판정. 일회성
  (sleep 1 → 확인 1 → 판정), 턴 기반. 루프 데몬 금지, 포그라운드 `sleep` 금지(막힘).
  간격: 부팅 5~8분, 학습 10~15분, stall 의심 시 촘촘히.
- **AI 리포트는 `#ai-리포팅` 채널로만** (`cloud/train/ai_report.py "msg"`; 숫자는
  `wandb_status.py` 로 prose 에 녹임) — 파드-push 채널(PIPELINE/STDOUT)과 분리. 폴링마다 추세
  코멘트 1회 OK, 시작·추세변화·완료·이상은 필수, 분 단위 도배 금지.

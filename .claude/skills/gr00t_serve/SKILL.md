...
파드 부팅·serve 기동은 하네스가 못 추적하는 외부 상태 — 직접 폴링한다(무과금 확인, 턴 기반).

- **부팅 폴링**: `Bash` `run_in_background` 로 `sleep 5~8분 && <확인>` → task-notification 으로
  읽고 다음 수 판정. 일회성(sleep 1 → 확인 1 → 판정), 포그라운드 `sleep` 금지(막힘).
- **serve 준비 신호 = 5555 바인딩** — `runpod_ls`/REST 의 `RUNNING` 은 컨테이너만 뜬 것이지
  서버 준비가 아니다(모델 다운로드+GPU 로드가 남음). 실제 준비는 5555 로 TCP connect 또는
  `PolicyClient.ping` 성공으로 확인.
- **불량 호스트 조기 감지** — 파드 생성 직후 `publicIp` 확인. `notes/cloud-ops-log.md` 에 기록된
  불량 호스트(`CUDA unknown error` 유발)와 같으면 부팅 완료를 기다리지 말고 즉시 `/runpod_down`

...
손목캠 v2-s3000 평가로 진단이 **결정적으로 검증**됐다. 50ep, seed=0, action_steps=16
(v1과 동일조건):

| | v1-s3000 (1캠) | v1-s20000 (1캠) | **v2-s3000 (손목캠)** |
|---|---|---|---|
| 성공률 | 0.20 | 0.32 | **0.48** |
| any_pick | — | 0.34 | 0.48 |
| correct_color | — | 0.94 | **1.00** (24/24) |
| 못 집은 실패 손→큐브 빗나감 | — | **8.4cm** | **2.0cm** |
| 학습 스텝 | 3000 | 20000 | 3000 |

- **같은 3000스텝에서 0.20→0.48 (2.4배).** 손목캠 v2가 **1캠을 20000스텝(0.32) 학습한 것보다도
  높다**(1/7 학습량). reaching 정밀도 천장 자체가 올라감.
- **결정적 증거**: 진단의 "단일 고정 카메라 깊이 모호성 → 8cm 빗나감"이 손목캠으로 **8.4→2.0cm**
  급감. 못 집은 26개도 평균 2cm 근접 실패(최소 0.3cm)지 v1의 gross 8cm 빗나감이 아님.
- 집은 건 **전부 성공**(any_pick=success=24), 색 **100%** → 유일한 잔여 실패 모드 = "2cm 근접
  실패". 스텝↑(v2-s20000)로 더 좁힐 여지.
- 주의: 50ep 단발이라 확산 정책 분산으로 절대값 CI는 유의하나, 0.20→0.48 + 빗나감 8.4→2.0cm는
  노이즈 초과 — 신호 견고. (모델: `adwel94/gr00t-threecubes@v2-s3000`, MANIFEST eval=0.48 기록.)
